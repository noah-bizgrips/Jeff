...
import { askGomez } from "@/lib/gomez/chat";
...
/**
 * POST /api/gomez/chat — owner + aal2 only. The Anthropic key never leaves the
...
export const POST = withErrorBoundary(async (req) => {
  ...
  try {
    ...
    const result = await askGomez(turns, { ownerId: g.session.userId, mode, request: req }, req);

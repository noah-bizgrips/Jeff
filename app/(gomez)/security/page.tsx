...
import { budgetStatus } from "@/lib/gomez/budget";
...
export default async function SecurityPage() {
  ...
  const [factors, owner, audit, connections, budget] = await Promise.all([
    ...
    budgetStatus(session.userId).catch(() => ({ spentUsd: 0, budgetUsd: 0, exhausted: false })),
  ]);
  const facts: SecurityFacts = {
    ...
    aiSpentTodayUsd: budget.spentUsd,
    aiBudgetUsd: budget.budgetUsd,

...
describe("store guards", () => {
  it("refuses to delete system jobs; user jobs can be created, updated and deleted", async () => {
    ...
    const created = await store.createJob(OWNER, { slug: "my-job", name: "My job", job_type: "user", schedule_type: "weekly", schedule_expression: "fri 07:05", sources: ["stripe"], detectors: ["failed_payment"] }, { createdBy: "owner" });

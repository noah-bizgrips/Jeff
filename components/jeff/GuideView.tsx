...
const SECTIONS: Section[] = [
  ...
  },
  {
    ...
    how: [
      ...
      "Add Job with a sentence (\"every Friday, find clients we do more work for than they pay us for\"); Jeff interprets it into sources, detectors and a schedule, and only creates it active when it is unambiguous and the sources are connected.",
      "Ask Jeff: \"What jobs are running?\", \"Test relationship radar\", \"Run the blind spot scanner\", \"Pause the expense creep hunter\", \"How are my jobs doing?\" (runs, failures, AI cost by job, false-positive rate).",
      "Jobs health (top of the Jobs page) shows the last 30 days: runs per day, success/partial/failed, AI calls and cost per job, findings created vs suppressed, feedback and Follow-Through activity. When you set aside the same kind of finding three times in two weeks, Jeff proposes a rule — pending in Memory & rules until you confirm it; your own rules always outrank proposals.",
    ],
    href: "/jobs",
  },
  {
    ...
    how: [
      "Press ✦ Find what I'm missing on Mission Control. Jeff reports progress (goals, business signals, commitments, obligations, financial, patterns, ranking) and then lists at most a handful of items, each with observed facts, why you might be missing it, evidence and confidence.",
      ...
      "Ranking is deterministic: impact × urgency × confidence × novelty × goal relevance × financial exposure × your rules. If nothing clears the bar Jeff says exactly that — \"I didn't find anything important enough to surface.\" — and sends no notification.",
      "Each item offers Ask Jeff about this, Prepare action (a mission draft you approve), Dismiss, and Create rule (a narrow rule anchored on that exact item, never the whole detector).",
    ],
    href: "/",
  },
  {
    ...
    what: "Open obligations Jeff tracks until they are actually resolved: reminders you asked for, promises detected in email and chat, client and portal deadlines, personal renewals. Jeff tracks resolution, not delivery — it never sends anything for you.",
    how: [
      "Ask Jeff: \"Remind me tomorrow to send Brian the proposal and keep reminding me until I do\" creates a persistent obligation with a due date, tracking mode, and what would count as completion. \"What's still waiting on me?\", \"What am I overdue on?\", \"Did I ever send Brian that proposal?\" answer from evidence.",
      "Natural phrasing works: \"Snooze the Calendly cancellation until Monday\", \"Mark the dentist thing done\", \"Stop reminding me about that\" (dismissed is not completed — Jeff says so). If two items match, Jeff asks which one.",
      "The Follow-Through queue groups overdue, waiting on you, waiting on others, possibly complete (Jeff found evidence and asks you to confirm), and snoozed. Every completion shows the evidence and the rule that satisfied it — \"Why did you mark this done?\" explains it.",

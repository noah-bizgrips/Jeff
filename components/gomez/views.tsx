...
import { retrieve, shortDate } from "@/lib/gomez/retrieve";
...
export function MemoriesView() {
  ...
  const docs = gomez.docs().filter((d) => (filter === "all" || d.source === filter) && (!collection || (d.tags ?? []).includes(collection)));
  return (
      ...
      <div className="saved-list">
        {gomez.saved.length ? (
          gomez.saved.map((a) => (
            <article className="saved-card" key={a.id}>
              ...
              <div className="saved-card-footer">
                ...
                <button className="text-button" type="button" onClick={() => gomez.deleteSaved(a.id)}>
                  <Icon name="trash" />
                  Remove
                </button>
              </div>
          ...
          <EmptyState
            ...
            action={
              <button className="button primary" type="button" onClick={() => gomez.setAgentOpen(true)}>
                Ask Gomez
              </button>
            }
          >
            Ask Gomez a question, then save an answer. It will be waiting right here.
          </EmptyState>
        )}
      </div>
    </section>

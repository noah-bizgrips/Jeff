...
import { SEED_DOCS, type GomezDoc } from "@/lib/gomez/demo-data";
import { GRAPH_SOURCES, SOURCES, sourceDef } from "@/lib/gomez/sources";
import { retrieve, extractiveAnswer } from "@/lib/gomez/retrieve";
...
import type { BrainState } from "@/lib/gomez/brain/state";
import { BRAIN_POLICY, resolveMotion } from "@/lib/gomez/brain/policy";
import { sourcesForTools } from "@/lib/gomez/brain/sources";
...
export interface GomezInitial {
  ...
  alertCount: number;
  /** What Gomez currently sees (read-only aggregate, spec §6). */
  brain: BrainState;
}

/**
 * Live activity the brain should show right now. `kind` non-null means Gomez is
...
interface GomezStore extends GomezInitial {
  ...
  /** Briefly illuminate sources Gomez actually read (≤1.5s), then clear. */
  flashBrainSources: (sources: string[]) => void;
}

const Ctx = createContext<GomezStore | null>(null);
...
export function useGomez(): GomezStore {
  const s = useContext(Ctx);
  if (!s) throw new Error("useGomez must be used inside GomezProvider");
  return s;
}

let toastSeq = 0;

export function GomezProvider({ initial, children }: { initial: GomezInitial; children: ReactNode }) {
  const router = useRouter();
  const pathname = usePathname();
  const [mode, setModeState] = useState<Mode>(initial.mode);
  const [connections, setConnections] = useState(initial.connections);
  const [notes, setNotes] = useState<GomezDoc[]>(initial.mode === "live" ? initial.notes : []);
  ...
  useEffect(() => {
    document.body.dataset.gomezMode = mode;
  }, [mode]);
  ...
  useEffect(() => {
    ...
    if (document.body.dataset.gomezMode === "live") noteAttention({ kind: "page_viewed", path: pathname });
  ...
  const ask = useCallback(
    async (text: string) => {
      ...
      setMessages(history);
      setBusy(true);
      // The brain shows Gomez investigating while the answer is pending; sources light up only once we know which were read.
      setBrainActivity({ kind: "ask", sources: [] });
      let used: string[] = [];
      try {
        const refs = retrieve(docs(), q, chatScope, 4);
        if (initial.aiEnabled) {
          const res = await fetch("/api/gomez/chat", {
            method: "POST",
          ...
          if (!res.ok || !data?.text) {
            const msg = data?.hint ?? (data?.error ? `Gomez could not answer (${data.error}).` : "Gomez could not answer right now.");
  ...
  const addNote = useCallback(
    async (title: string, content: string, tags: string[]) => {
      if (looksSensitiveClient(`${title} ${content}`)) {
        toast("Possible credential or private access link detected. Do not add secrets to Gomez.");
        return false;
      }
      const note: GomezDoc = {

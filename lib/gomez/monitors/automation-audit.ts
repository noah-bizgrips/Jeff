...
export function repeatedError(rows: SourceRow[], ctx: ExtendedContext): CandidateFinding[] {
  ...
  for (const [wf, list] of groupBy(errors, (r) => str(r.metadata.workflowId) ?? r.title)) {
    ...
    out.push({
      ...
      severity: "high",
      proposed_mission: { title: `Repair workflow ${list[0]!.title ?? wf}`, goal: `Diagnose the recurring failure in n8n workflow ${wf} and prepare a fix in a gomez-test copy. Do not modify production workflows.` },
    });
  }
  return out;
}
...
export function manualRepetition(rows: SourceRow[], ctx: ExtendedContext): CandidateFinding[] {
  ...
  for (const [key, list] of groups) {
    ...
    out.push({
      ...
      severity: "low",
      proposed_mission: { title: `Automate "${list[0]!.title}"`, goal: `Analyse the ${list.length} occurrences of this task and propose a template or n8n workflow draft (gomez-test only).` },
    });
  }
  return out;
}

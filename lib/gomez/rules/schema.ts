...
export const RuleConditionSchema = z
  .object({
    ...
    /** True = only leads that belong to a client-portal client (see lib/gomez/clients/client-leads.ts); false = only everything else. */
...
export const RuleInputSchema = z
  .object({
    ...
    /** Job slug: the rule applies only inside that job's runs (Gomez's Jobs). */

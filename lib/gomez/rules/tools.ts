...
/**
 * Ask Gomez tools for memories and operating rules. These are the only way
 * the model can change Gomez's behaviour, and every change goes through the
...
export const MEMORY_RULE_TOOLS: Anthropic.Beta.BetaTool[] = [
  {
    ...
    description:
      "Stores a durable soft memory about how the owner thinks, works, prioritises or wants Gomez to behave (e.g. 'prefers reports under 10 bullets', 'pipeline value is not revenue'). Use for 'remember that…' statements and preferences that are not deterministic rules.",
    input_schema: {
      type: "object",
      properties: {
  ...
  {
    name: "list_memories",
    description: "Lists what Gomez remembers about the owner (preferences, definitions, priorities, dislikes).",
  ...
  },
  {
    name: "interpret_rule",
    description:
      "Turns behavioural feedback in natural language ('stop flagging GitHub emails as commitments', 'never alert me about failed payments under $50', 'actually, alert me about failed production deploys') into a proposed structured operating rule, memory, or forget request. ALWAYS call this first for feedback about what Gomez should or should not show/flag/alert; then call apply_rule with the returned rule. Pass the owner's exact sentence.",
    input_schema: {
      type: "object",
      properties: {

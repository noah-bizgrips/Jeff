...
import { loadClientLeadIndex } from "@/lib/gomez/clients/client-leads-store";
import type { CandidateFinding, EvidenceRef, SourceRow } from "@/lib/gomez/monitors/types";
...
const SYSTEM_RULE_SEEDS: { name: string; input: Record<string, unknown>; reprocess: boolean }[] = [
  ...
  },
  {
    ...
    input: {
      name: CLIENT_LEAD_RULE_NAME,
      description:
        "A lead that came in through a client's lead sources (a homeowner wanting a bathroom) is the client's to follow up, not yours. Gomez only applies this to leads it can trace to a client portal record (portal lead ↔ HighLevel contact id, hashed email, phone + client, or a lead-source routing key); agency leads — people who might hire BizGrips — are still flagged. Disable this rule to be nagged about client leads again.",
      rule_type: "monitor_filter",
      scope: "business",
      target_system: "monitors",
      target_monitor: null,

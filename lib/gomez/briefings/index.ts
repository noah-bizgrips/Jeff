...
import { listAlerts } from "@/lib/gomez/alerts/store";
import { listCommitments } from "@/lib/gomez/commitments/store";
import { listObligations } from "@/lib/gomez/obligations/store";
import { bucketOf } from "@/lib/gomez/obligations/types";
import { loadFreshness } from "@/lib/gomez/freshness-store";
import { loadRows } from "@/lib/gomez/monitors";
import { plaidFlows, stripeFlows, inWindow, num } from "@/lib/gomez/monitors/finance-shared";
import { latestSnapshot, listGoalEvents, listGoalMetrics, listGoals } from "@/lib/gomez/goals/store";
import { formatMetricValue, formatTarget } from "@/lib/gomez/goals/format";
import { TRAJECTORY_LABEL } from "@/lib/gomez/goals/schema";
import { label as trajectoryWord } from "@/lib/gomez/alerts/engine";
...
import { shouldPushBriefing } from "@/lib/gomez/push/decide";
...
export async function buildBundle(ownerId: string, kind: BriefingKind, period: { period_start: string; period_end: string }, now = new Date()): Promise<BriefingBundle> {
  ...
  const { groupMembershipForObligations } = await import("@/lib/gomez/grouping/store");

...
import { GOMEZ_TOOLS, runTool, type ToolContext } from "./tools";
 ...
 */
export const GOMEZ_SYSTEM_PROMPT = `You are Gomez, the private second brain and operations coordinator for BizGrips, a small business run by Noah.
...
- When the owner gives behavioural feedback — "stop showing X", "don't flag Y as Z", "never alert me about…", "only alert me when…", "actually, do alert me about…", "I keep getting … in monitors, I don't want that" — you MUST change Gomez's behaviour: call interpret_rule with the owner's exact sentence, then apply_rule with the returned rule. Never answer "I can't change that" and never merely offer a mission; rules are how Gomez changes.
...
- "Find something we're doing stupidly" / "where are we wasting money or time": call get_findings and get_alerts and present findings with observed facts → calculated metrics → interpretation, plus what Gomez can prepare. Never present pipeline value as lost revenue.
...
- Grouped alerts: when get_alerts returns kind "group" (e.g. "Pure Bath of Michigan — 6 overdue portal tasks") treat it as ONE situation. Call get_alert_groups with its group_id when the owner asks what is in it, what is going on with a client, or what the underlying problem is; present the deterministic summary (oldest overdue, owner split, primary blocker), Gomez's interpretation if present, then the members. Never list the hidden member alerts as separate problems. "Remind <client>" / "prepare an action" on a group → offer the draft-only mission from the Alerts view; nothing is sent.
- Owner asks to snooze/dismiss/acknowledge an alert → update_alert (or get_alert_groups with action for a grouped situation). Owner asks to change brief time, quiet hours, timezone or notification thresholds → update_settings (Tier 1 only) and confirm the exact change. Security settings are never changeable through chat.

Gomez's Jobs (recurring analysts Gomez owns — perform, don't explain):
...
Follow-Through (open obligations — Gomez tracks resolution, not delivery):
...
export async function askGomez(turns: ChatTurn[], ctx: ToolContext, request?: Request): Promise<ChatResult> {
  const budget = await budgetStatus(ctx.ownerId);
  if (budget.exhausted) {
    return {
      text: `Gomez has reached today's AI budget ($${budget.budgetUsd.toFixed(2)}). I'll be back tomorrow (UTC), or raise JEFF_DAILY_BUDGET_USD in Vercel. Search and your saved answers still work.`,
      toolsUsed: [],
      citations: [],
      model: JEFF_MODEL,
  ...
  for (let iteration = 0; iteration < 6; iteration++) {
    ...
    const response = await client.beta.messages.create({
      ...
      system: [{ type: "text", text: GOMEZ_SYSTEM_PROMPT, cache_control: { type: "ephemeral" } }],
      tools: GOMEZ_TOOLS,
    ...
    if (response.stop_reason !== "tool_use" || toolUses.length === 0) {
      const text = response.content
        ...
        .join("\n")
        .trim();
      await audit({ event: "gomez_chat", ownerId: ctx.ownerId, request, metadata: { tools: toolsUsed, model: response.model, turns: turns.length, estimatedUsd: spentThisRequest } });
    ...
    for (const tu of toolUses) {
      ...
      try {
        ...
        log.warn("gomez_tool_failed", { tool: tu.name, message: errorMessage(err) });
      }

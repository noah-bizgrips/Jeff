...
export function extractPeople(text: string): string[] {
  ...
  for (const m of text.matchAll(/\b(?:to|for|from|with)\s+([A-Z][a-z]+(?:\s+[A-Z][a-z]+)?)\b/g)) {
    const name = m[1]!;
    if (!/^(Friday|Monday|Tuesday|Wednesday|Thursday|Saturday|Sunday|Gomez|Stripe|Notion|Gmail|Slack|Calendly)$/.test(name)) out.add(name);
  }
...
export function inferStrategy(text: string, people: string[]): { strategy: CompletionStrategy; uncertain: boolean } {
  ...
  return { strategy: base("manual", {}, "Only you can mark this done — Gomez has no reliable evidence source for it", 1), uncertain: true };
}

function cleanTitle(text: string): string {
  let t = text.trim();
  t = t.replace(/^(hey |ok |please |gomez,? |can you )+/i, "");
...
export function interpretReminder(text: string, now: Date, timezone: string): ObligationInterpretation {
  ...
  if (!due.iso) ambiguities.push("No due date found — Gomez will treat it as 'soon' and remind in the next brief.");
  if (uncertain && strategy.kind !== "manual") ambiguities.push("Completion evidence may be ambiguous; Gomez will ask before closing it.");
...
export function toObligationInput(i: ObligationInterpretation, origin = "gomez"): ObligationInput {
  return {
    title: i.title,
    description: null,
    scope: i.scope,

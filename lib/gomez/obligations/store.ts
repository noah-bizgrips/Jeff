...
export async function createObligation(ownerId: string, input: ObligationInput, opts: { actor?: "owner" | "system" | "gomez"; now?: Date } = {}): Promise<{ row: ObligationRow; created: boolean }> {
...
/**
 * Owner/Gomez actions. Complete ≠ dismiss ≠ cancel ≠ snooze — every transition
 * is recorded so history is never lost.
 */
export async function applyAction(ownerId: string, id: string, action: ObligationActionInput, opts: { actor?: "owner" | "gomez"; now?: Date; request?: Request } = {}): Promise<ObligationRow | null> {
  ...
  if (row) {
    await audit({ event: "obligation_updated", ownerId, actor: opts.actor === "gomez" ? "system" : "owner", targetId: id, request: opts.request, metadata: { action: action.action, status: row.status } });

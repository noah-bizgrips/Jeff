...
/**
 * Ask Gomez tools for Follow-Through. "Stop reminding me" dismisses (never
...
export const OBLIGATION_TOOLS: Anthropic.Beta.BetaTool[] = [
  ...
  },
  {
    ...
    description: "Explains why Gomez marked an obligation done (or why it is still open): the evidence record, source and completion rule. Use for 'why did you mark this done?'.",
...
export async function runObligationTool(name: string, input: Record<string, unknown>, ctx: ObligationToolContext): Promise<unknown> {
  ...
  switch (name) {
    case "create_reminder": {
      ...
      const { row, created } = await createObligation(ctx.ownerId, toObligationInput(interp, "gomez"), { actor: "owner", now });

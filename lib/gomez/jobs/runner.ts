...
import { loadRows, persistFindings } from "@/lib/gomez/monitors";
import type { CandidateFinding, GoalLite, SourceRow } from "@/lib/gomez/monitors/types";
...
import { refreshGoals, type GoalRefreshResult } from "@/lib/gomez/goals/refresh";
import { latestSnapshot } from "@/lib/gomez/goals/store";
import { TRAJECTORY_LABEL } from "@/lib/gomez/goals/schema";
import { formatMetricValue } from "@/lib/gomez/goals/format";
import { loadBlindSpotContext, loadNoveltyInputs, applyNovelty, runBlindSpotsForOwner, detectBlindSpots, DETECTORS as BLIND_SPOT_DETECTORS, EMPTY_RESULT_MESSAGE } from "@/lib/gomez/blindspots";
import { toFinding as blindSpotToFinding } from "@/lib/gomez/blindspots/detect";
...
export async function loadDetectorExtras(ownerId: string, specs: DetectorSpec[], now: Date): Promise<Pick<DetectorContext, "ownerEmail" | "goals" | "memories" | "obligations" | "timezone">> {
  ...
  if (needs.has("goals")) {
    const { listGoals, listGoalMetrics, listRecommendations } = await import("@/lib/gomez/goals/store");
  ...
  if (needs.has("memories")) {
    try {
      const { listMemories } = await import("@/lib/gomez/rules/store");
...
export async function runJob(ownerId: string, job: JobRow, opts: { mode: RunMode; now?: Date; onStart?: (runId: string) => void }): Promise<JobRunOutcome> {
  ...
  try {
    ...
    if (specials.includes("follow_through")) {
      ...
      const { runFollowThrough } = await import("@/lib/gomez/obligations/watchdog");
...
async function refreshGoalsReadOnly(ownerId: string, now: Date): Promise<GoalRefreshResult[]> {
  const { listGoals } = await import("@/lib/gomez/goals/store");
  const goals = await listGoals(ownerId, ["active"]).catch(() => []);

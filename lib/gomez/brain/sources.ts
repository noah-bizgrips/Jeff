 ...
 * stages and the brain's visual source ids (lib/gomez/sources.ts).
 * Deterministic: nothing here guesses.
 */
import { SOURCES } from "@/lib/gomez/sources";
...
/** Ask Gomez tool names → sources they actually read. */
  ...
  for (const t of tools) {
    switch (t) {
      ...
      case "get_client_overview":
        ...
        break;
      default:
        break; // memory/rules/goals/alerts/briefings/jobs tools read Gomez's own tables, not a source
    }
  }
  return [...out];
}

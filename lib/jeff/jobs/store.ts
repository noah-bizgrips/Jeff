...
export async function createJob(ownerId: string, raw: unknown, meta: { createdBy: "owner" | "jeff"; systemManaged?: boolean }): Promise<{ ok: true; job: JobRow } | { ok: false; reason: string }> {
  ...
  const { data, error } = await admin
    .from("jobs")
    .insert({ ...parsed.data, owner_id: ownerId, system_managed: meta.systemManaged ?? false, created_by: meta.createdBy, next_run_at: nextRunAt(parsed.data, new Date(), tz)?.toISOString() ?? null })
    .select(JOB_COLUMNS)
    .single();

 ...
 * Rules). The one-line suggestion lives in `source_quote` so Ask Jeff and
...
export async function runJobLearning(ownerId: string, now = new Date()): Promise<LearningRunSummary> {
  ...
  for (const p of proposals) {
    const res = await createRule(ownerId, p.rule, { source: "feedback", createdBy: "jeff", pendingConfirmation: true, sourceQuote: p.suggestion });
    if (res.ok) {
...
/** Pending learned proposals, as one-liners for Ask Jeff and the brief. */

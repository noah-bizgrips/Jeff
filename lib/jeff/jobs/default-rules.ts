...
import type { OperatingRule, RuleInput } from "@/lib/jeff/rules/schema";
 ...
 * owner writes (created_by "jeff", source "settings" → preference class).
 *
...
export const JOB_DEFAULT_RULES: JobDefaultRule[] = [
  ...
  },
  {
    ...
    description: "Only propose automating something Jeff has seen enough times to be sure it repeats.",
    rule_type: "monitor_filter",
    scope: "business",
    target_system: "monitors",
  ...
  },
  {
    ...
    description: "Blind spots are speculative by nature; anything Jeff is less than 40% sure about is dropped rather than shown.",
    rule_type: "monitor_filter",
    scope: "all",
    target_system: "monitors",
...
export async function ensureJobDefaultRules(ownerId: string, jobs: JobRow[], existing?: OperatingRule[]): Promise<{ created: number }> {
  ...
  for (const job of todo) {
    for (const def of defaultRulesFor(job.slug)) {
      ...
      const res = await createRule(ownerId, { ...input, target_job: slug }, { source: "settings", createdBy: "jeff", pendingConfirmation: false });
      if (res.ok) created++;

...
import { CLASSIFIER_VERSION, classifyCommitment } from "@/lib/jeff/monitors/commitment-classifier";
import { listRules } from "@/lib/jeff/rules/store";
import { decide } from "@/lib/jeff/rules/precedence";
import { bareAddress, subjectFromRow } from "@/lib/jeff/rules/engine";
import { ClientLeadIndex } from "@/lib/jeff/clients/client-leads";
import { ownerIdentity } from "@/lib/env";
import { patchObligation, recordEvent } from "@/lib/jeff/obligations/store";
import { TERMINAL_STATUSES } from "@/lib/jeff/obligations/types";
...
export async function reclassifyCommitments(ownerId: string, now = new Date()): Promise<ReclassifySummary> {
  ...
  for (const c of open) {
    ...
    for (const o of obs ?? []) {
      ...
      await recordEvent(ownerId, String(o.id), "note", { actor: "jeff", note: RECLASSIFY_NOTE, reason, classifier_version: CLASSIFIER_VERSION }, now);

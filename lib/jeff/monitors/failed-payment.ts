...
export const failedPayment: Monitor = {
  ...
  run(rows, ctx) {
    ...
    const finding: CandidateFinding = {
      ...
      interpretation:
        "Interpretation: money that is owed or that bounced. Past-due invoices usually need a friendly reminder or an updated card; repeated failed charges on the same customer often indicate an expired card. Jeff will not contact customers — it can prepare the list.",

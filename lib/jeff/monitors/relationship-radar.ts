 ...
 * adapter (lib/jeff/obligations/relationship-source.ts) so nothing is tracked twice.
 */

export const QUIET_MIN_DAYS = 21;
export const QUIET_GAP_MULTIPLIER = 2;
export const RESURFACE_SILENCE_DAYS = 90;
export const REFERRAL_RECENT_DAYS = 60;
export const REFERRAL_PRIOR_DAYS = 240;
...
export function importantDate(rows: SourceRow[], ctx: ExtendedContext): CandidateFinding[] {
  ...
  for (const r of rows) {
    ...
    out.push({
      ...
      confidence: 0.9,
      limitations: "Only events you named as birthday/anniversary; Jeff does not infer dates from other data.",
      severity: "info",
      proposed_mission: null,
    });
  }

  ...
  run(rows, ctx) {
    ...
    for (const [clientId, invoices] of groupBy(unpaid, (r) => str(r.metadata.client_id))) {
      ...
      out.push({
        ...
        interpretation: `Interpretation: ${name} is behind on payment. If work is still being delivered for them, this is the moment to pause new deliverables or send a reminder — Jeff will not contact them.`,

...
import { isPublicMailboxDomain } from "@/lib/jeff/clients/map-core";
  ...
  run(ctx: BlindSpotContext): BlindSpotCandidate[] {
    ...
    for (const c of ctx.clients) {
      ...
      out.push({
        ...
        interpretation: c.stripe_customer_ids.length ? "Either the client stopped being billed, or billing moved somewhere Jeff can't see. Both are worth a look." : "Jeff can't match this client to a Stripe customer by email, so either billing is off-platform or the attribution needs a manual link.",
